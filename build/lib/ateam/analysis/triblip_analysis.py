...
def df_marginal_apply(df, xcol, ycol, data_col, fcn):
    ...
    sig_frac_x = pd.concat([sig_frac_x], keys=['All'], names=[ycol]).reorder_levels([1,0])
...
def plot_sweeps_prop(tri_df, cell, propname, show_burst=True):
    ...
    plt.scatter(celldat[celldat.is_bursty].frequency, celldat[celldat.is_bursty][propname], c='r')
    if show_burst:
    	plt.legend(['Regular','Bursting'])
    for sweep in celldat.index:
        plt.annotate(sweep, (celldat.frequency[sweep], celldat[propname][sweep]))
    # plt.ylim(ymin=0)
...
def plot_pop_dist(series_trend):
    ...
    trend_mean, trend_std = tri.pop_stats_robust(series_trend)
    v_cut = trend_mean + 3*trend_std
    hist_normal = 1/np.sqrt(2*np.pi*trend_std**2) * np.exp(-(xx-trend_mean)**2/(2*trend_std**2))
    plt.plot(xx, hist_normal)
    plt.axvline(v_cut, color='r')
    plt.legend(['Normal distribution', '3$\sigma$ cutoff'])