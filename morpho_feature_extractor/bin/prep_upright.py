...
def fetch_json(specimen_id, cursor):
    sql = """
        SELECT DISTINCT
    ...
    res = cursor.fetchall()
    if len(res) > 3:
        logging.warning("too many polygons returned. Expected 3, got %d", len(res))
        for blk in res:
            logging.warning(blk)
        logging.warning("--------------------")
    ...
    for blk in res:
        ...
        if 'Soma' not in jout and name == 'Soma':
            ...
            jout[name]["resolution"] = blk["resolution"]
    if len(jout) != 3:
        raise Exception("Failed to fetch necessary polygons -- got %d of 3" % len(jout))
    #
...
def calculate_transform(cursor, specimen_id, morph):
    try:
        jin = fetch_json(specimen_id, cursor)
        soma = jin["Soma"]["path"]
        pia = jin["Pia"]["path"]
        ...
        if root is None:
            logging.warning("using root instead of soma")
            root = morph.root()
        ...
        logging.error("Error calculating transform: %s", e.message)
        return None
    return tr_rot
