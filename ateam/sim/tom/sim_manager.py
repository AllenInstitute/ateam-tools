...
### Configure inputs and modules
    ...
    def run_bionet(self):
        self.config.save()
        self._run_bionet(self.config.path)
        

    def run_bionet_mpi(self, ncores=1):
        self.config.save()
        import subprocess
        config = os.path.basename(self.config.path)
        command = "mpirun -np {ncores} nrniv -mpi -python -c \"import ateam.sim.tom as sim; sim.SimManager._run_bionet(\\\"{config}\\\")\"".format(
            config=config, ncores=ncores)
